...
Add a local Runtime Doctor endpoint at `GET /api/runtime/doctor` and expose it through a Runtime page in the web control center. The doctor reports required overlay files, preferred platform-specific overlay virtual environment, Python imports, writable local paths, platform permission hints, and best-effort companion service health.
