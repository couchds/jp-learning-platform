...
- Electron captures the display under the pointer and briefly hides the main window so Kakomu is not included in its own screenshot.
- `Ctrl/Cmd+Shift+O` sends the captured image to the shared React renderer and opens the Capture page.
- Closing the main window hides it to the system tray so the global shortcut and managed OCR workers remain available. The tray menu can open Kakomu, start a capture, or quit the app and its services.
...
- macOS may require Screen Recording permission for Kakomu. Windows and Linux use the platform desktop capture implementation.
